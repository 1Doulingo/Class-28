...
function preload() {
  ...
  canonExplosionSound = loadSound("assets/cannon_explosion.mp3")
  canonWaterSound = loadSound("assets/cannon_water.mp3")
...
function keyPressed() {
  if (keyCode === DOWN_ARROW) {
    canonExplosionSound.play();
    canonExplosionSound.setVolume(0.1);
...
function showCannonBalls(ball, index) {
  if (ball) {
    ...
    if (ball.body.position.x >= width || ball.body.position.y >= height - 50) {
     ...
     if (!ball.isSink){
      canonWaterSound.play();
      canonWaterSound.setVolume(0.1);
      ball.remove(index);
     } 
    }
  }
